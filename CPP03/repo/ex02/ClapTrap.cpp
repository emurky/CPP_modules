...
ClapTrap::ClapTrap( ClapTrap const & src ) {
	...
	*this = src;
	...
	std::cout	<< "ClapTrap copy constructor was called for "
				<< this->_name << std::endl;
...
void	ClapTrap::takeDamage( unsigned int amount ) {
	...
	if ( _energy_pts > amount ) {
		...
		_energy_pts = 0;
		std::cout	<< _className << " " << _name << " takes damage "
...
void	ClapTrap::beRepaired( unsigned int amount ) {
	...
	_energy_pts += amount;
	std::cout	<< _className << " " << _name << " gains " << amount
...
void			ClapTrap::addHitpoints( unsigned int amount ) {
...
void	ClapTrap::printStatus( std::string className ) const {

	std::cout	<< className << " status" << std::endl
				...
				<< std::setw( 15 ) << std::left
				<< "Attack damage: " << _attack_dmg << std::endl << std::endl;
